...
class WiBehavior(TaskSet):
    ...
    taskSpaceId = ""
    taskSpaceIndex = -1
    spaceIndexLock = threading.RLock()
    # taskUserName = ""
    taskUserToken = ""
    ...
    def nextSpaceIndex(self):
        self.spaceIndexLock.acquire()
        self.taskSpaceIndex = self.taskSpaceIndex + 1
        self.spaceIndexLock.release()
        return self.taskSpaceIndex
    ...
    def crudSpace(self):
        # CREATE
        si = str(self.taskUser) + "-" + str(self.nextSpaceIndex()) + "-" + str(uuid.uuid4());
        space = {
            "data": {
                "type": "spaces",
                "attributes": {
